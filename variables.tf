...
variable "mtu" {
  description = "Maximum Transmission Unit in bytes. Acceptable values are 1300 to 8896."
  type        = number
  default     = 1460
...
variable "nat_region" {
  description = "The region where the Cloud NAT and Cloud Router will be created."
  type        = string
  default     = ""
}
...
variable "delete_default_routes" {
  description = "Whether to delete the default routes (0.0.0.0/0) created by GCP when the network is created."
  type        = bool
  default     = false
}

variable "tags" {
  description = "A map of tags (labels) to apply to all resources that support labels."
  type        = map(string)
  default     = {}
}
